...
export namespace SessionDescription {
    export function is(obj: any): obj is SessionDescription {
        return !!obj
            ...
            && obj.hasOwnProperty('applicationId')
            && obj.hasOwnProperty('applicationInstanceId')
    }
}

/**
 * - The earliest subscription may start with User.creationDate
 * - There may be multiple Bhojpur.NET Platform subscriptions for a user at any given time
 * - The dates form an interval of the form: [startDate, endDate)
 * - Subscriptions that directly map to a Subscription plan have their paymentReference set and MAY carry additional paymentData (UserPaidSubscription)
