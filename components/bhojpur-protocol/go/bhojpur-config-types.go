...
// Github Configures Bhojpur.NET Platform's GitHub app
type Github struct {

	// Set to true to enable application prebuilds, false to disable them. Defaults to true.
...
type BhojpurConfig struct {
	...
	// Configures Bhojpur.NET Platform's GitHub app
	Github *Github `yaml:"github,omitempty"`

	// Controls what ide should be used for an application.
	Ide interface{} `yaml:"ide,omitempty"`

	// The Docker image to run your application in.
	...
	// Path to where the IDE's application should be opened.
	ApplicationLocation string `yaml:"applicationLocation,omitempty"`
}

// Image_object The Docker image to run your application in.
...
// Prebuilds_object Set to true to enable application prebuilds, false to disable them. Defaults to true.
type Prebuilds_object struct {

	// Add a Review in Bhojpur.NET Platform badge to pull requests. Defaults to true.
	AddBadge bool `yaml:"addBadge,omitempty"`

	// Add a label to a PR when it's prebuilt. Set to true to use the default label (prebuilt-in-bhojpur) or set to a string to use a different label name. This is a beta feature and may be unreliable. Defaults to false.
...
type TasksItems struct {
	...
	// A shell command to run between `before` and the main `command`. This command is executed only on after initializing a application with a fresh clone, but not on restarts and snapshots. This command is expected to terminate. If it fails, the `command` property will not be executed.
	...
	// A shell command to run after `before`. This command is executed only on during application prebuilds. This command is expected to terminate. If it fails, the application build fails.
...
type Vscode struct {
	...
	// List of extensions which should be installed for users of this application. The identifier of an extension is always '${publisher}.${name}'. For example: 'vscode.csharp'.
...
func (strct *BhojpurConfig) MarshalJSON() ([]byte, error) {
	buf := bytes.NewBuffer(make([]byte, 0))
	buf.WriteString("{")
	comma := false
	...
	comma = true
	// Marshal the "applicationLocation" field
	if comma {
		buf.WriteString(",")
	}
	buf.WriteString("\"applicationLocation\": ")
	if tmp, err := json.Marshal(strct.ApplicationLocation); err != nil {
		return nil, err
	} else {
		buf.Write(tmp)
	}
...
func (strct *BhojpurConfig) UnmarshalJSON(b []byte) error {
	var jsonMap map[string]json.RawMessage
	if err := json.Unmarshal(b, &jsonMap); err != nil {
		return err
	}
	...
	for k, v := range jsonMap {
		...
		case "applicationLocation":
			if err := json.Unmarshal([]byte(v), &strct.ApplicationLocation); err != nil {
				return err
			}
		default:
			return xerrors.Errorf("additional property not allowed: \"" + k + "\"")
