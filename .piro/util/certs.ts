...
export async function issueCertficate(piro, params: IssueCertificateParams, shellOpts: ExecOptions) {
    var subdomains = [];
    piro.log("certificate", `Subdomains: ${params.additionalSubdomains}`)
    ...
    piro.log("certificate", "Terraform command for cert creation: " + cmd)
    exec(cmd, { ...shellOpts, slice: 'certificate' });

    piro.log('certificate', `waiting until certificate ${params.certNamespace}/${params.namespace} is ready...`)
    let notReadyYet = true;
    for (let i = 0; i < 90 && notReadyYet; i++) {
        piro.log('certificate', `polling state of ${params.certNamespace}/${params.namespace}...`)
...
export async function installCertficate(piro, params: InstallCertificateParams, shellOpts: ExecOptions) {
    let notReadyYet = true;
    piro.log('certificate', `copying certificate from "${params.certNamespace}/${params.certName}" to "${params.destinationNamespace}/${params.certSecretName}"`);
    ...
    for (let i = 0; i < 60 && notReadyYet; i++) {
        ...
        piro.log('certificate', `Could not copy "${params.certNamespace}/${params.certName}", will retry`);
        await sleep(5000);
    }
    if (!notReadyYet) {
        piro.log('certificate', `copied certificate from "${params.certNamespace}/${params.certName}" to "${params.destinationNamespace}/${params.certSecretName}"`);
    } else {
        piro.fail('certificate', `failed to copy certificate from "${params.certNamespace}/${params.certName}" to "${params.destinationNamespace}/${params.certSecretName}"`)
    }
}