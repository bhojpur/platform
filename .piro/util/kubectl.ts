...
export const helmInstallName = "bhojpur";
...
export async function wipePreviewEnvironmentAndNamespace(helmInstallName: string, namespace: string, shellOpts: ExecOptions) {
    ...
    deleteAllApplications(namespace, shellOpts);
...
export async function wipePreviewEnvironmentHelm(helmInstallName: string, namespace: string, shellOpts: ExecOptions) {
    ...
    //  - bp-scaler can't create new ghosts in the meantime
    //  - bp-manager can't start new probes/applications
...
async function wipePreviewEnvironmentInstaller(namespace: string, shellOpts: ExecOptions) {
    ...
    const hasBhojpurConfigmap = (exec(`kubectl -n ${namespace} get configmap bhojpur-app`, { slice, dontCheckRc: true })).code === 0;
    if (hasBhojpurConfigmap) {
        piro.log(slice, `${namespace} has Bhojpur.NET Platform configmap, proceeding with removal`);
        const inPiroFolder = exec(`pwd`, { slice, dontCheckRc: true }).stdout.trim().endsWith(".piro");
        if (inPiroFolder) {
            // used in .piro/wipe-devstaging.yaml on preview environment clean-up
            exec(`./util/uninstall-bhojpur.sh ${namespace}`, { slice });
        } else {
            // used in .piro/build.yaml on 'with-clean-slate-deployment=true'
            exec(`./.piro/util/uninstall-bhojpur.sh ${namespace}`, { slice });
        }

    } else {
        piro.log(slice, `There is no Bhojpur.NET Platform configmap, moving on`);
    }
}
...
// Delete pods for running applications, even if they are stuck in terminating because of the finalizer decorator
function deleteAllApplications(namespace: string, shellOpts: ExecOptions) {
    const objs = exec(`kubectl get pod -l component=application --namespace ${namespace} --no-headers -o=custom-columns=:metadata.name`, { ...shellOpts, async: false })
        .split("\n")
        .map(o => o.trim())
        .filter(o => o.length > 0);
    ...
    objs.forEach(o => {
        try {
            // In most cases the calls below fails because the application is already gone. Ignore those cases, log others.
...
async function deleteAllUnnamespacedObjects(namespace: string, shellOpts: ExecOptions): Promise<void> {
    ...
    for (const resType of ["clusterrole", "clusterrolebinding", "podsecuritypolicy"]) {
        piro.log(slice, `Searching and filtering ${resType}s...`);
        ...
        piro.log(slice, `Deleting old ${resType}s...`);
...
export async function deleteNonNamespaceObjects(namespace: string, destname: string, shellOpts: ExecOptions) {
    exec(`/usr/local/bin/helm3 delete bhojpur-${destname} || echo bhojpur-${destname} was not installed yet`, { ...shellOpts });

    let objs = [];
    ["ws-scheduler", "node-daemon", "cluster", "application", "jaeger", "jaeger-agent", "bp-sync", "bp-manager-node", "bp-daemon", "registry-facade"].forEach(comp =>
...
export function findFreeHostPorts(ranges: PortRange[], shellOpts: ExecOptions): number[] {
    ...
    piro.log(shellOpts.slice, `already reserved ports: ${Array.from(alreadyReservedPorts.values()).map(p => ""+p).join(", ")}`);
