...
# 4. call this script like so: ./.piro/post-process.sh 1234 5678 2 your-branch-just-dashes
...
while [ "$i" -le "$DOCS" ]; do
   ...
   APPLICATION_COMPONENTS=("image-builder" "image-builder-mk3 blobserve registry-facade" "bp-daemon" "agent-smith")
   # shellcheck disable=SC2076
   if [[ " ${APPLICATION_COMPONENTS[*]} " =~ " ${NAME} " ]] && { [[ "$KIND" == "Deployment" ]] || [[ "$KIND" == "DaemonSet" ]]; }; then
      LABEL="bhojpur.net/application_$NODE_POOL_INDEX"
      ...
      LABEL="bhojpur.net/workload_meta"
      echo "setting $LABEL for $NAME"
      touch /tmp/"$NAME"pool.yaml
      # create a matching expression
      yq w -i /tmp/"$NAME"pool.yaml spec.template.spec.affinity.nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution.nodeSelectorTerms[0].matchExpressions[0].key "$LABEL"
   ...
   if [[ "server-config" == "$NAME" ]] && [[ "$KIND" == "ConfigMap" ]]; then
      ...
      THEIA_BUCKET_NAME=$(yq r ./.piro/values.dev.yaml components.server.theiaPluginsBucketNameOverride)
      ...
      # is expected to look like ws-dev.<branch-name-with-dashes>.staging.bhojpur.net
      SHORT_NAME=$(yq r ./.piro/values.dev.yaml installation.shortname)
      NAMESPACE=$(kubens -c)
      ...
      # Stage
      STAGE=$(yq r ./.piro/values.dev.yaml installation.stage)
      ...
      # default none, this is CSV list like: ws-feature-flags=registry_facade,full_application_backup
      if [ "${#DEFAULT_FEATURE_FLAGS}" -gt 0 ]; then
         ...
         jq '.applicationDefaults.defaultFeatureFlags += $flags' /tmp/"$NAME"overrides.json --slurpfile flags /tmp/defaultFeatureFlags > /tmp/"$NAME"-updated-overrides.json
   ...
   if [[ "ws-manager" == "$NAME" ]] && [[ "$KIND" == "ConfigMap" ]]; then
      ...
      SHORT_NAME=$(yq r ./.piro/values.dev.yaml installation.shortname)
      STAGING_HOST_NAME=$(yq r ./.piro/values.dev.yaml hostname)
      CURRENT_WS_HOST_NAME="ws.$DEV_BRANCH.$STAGING_HOST_NAME"
      NEW_WS_HOST_NAME="ws-$SHORT_NAME.$DEV_BRANCH.$STAGING_HOST_NAME"

      WS_CLUSTER_HOST_EXPR="s/\"applicationClusterHost\": \"$CURRENT_WS_HOST_NAME\"/\"applicationClusterHost\": \"$NEW_WS_HOST_NAME\"/"
      sed -i "$WS_CLUSTER_HOST_EXPR" /tmp/"$NAME"overrides.yaml

      WS_PORT_URL_TEMP_EXPR="s|\"portUrlTemplate\": \"https://{{ .ApplicationPort }}-{{ .Prefix }}.$CURRENT_WS_HOST_NAME\"|\"portUrlTemplate\": \"https://{{ .ApplicationPort }}-{{ .Prefix }}.$NEW_WS_HOST_NAME\"|"
      ...
      # is expected to be reg.<branch-name-with-dashes>.staging.bhojpur.net:$REG_DAEMON_PORT
      # Change the port we use to connect to ws-daemon
      REGISTRY_FACADE_HOST="reg.$DEV_BRANCH.staging.bhojpur.net:$REG_DAEMON_PORT"
   ...
   if [[ "ws-proxy" == "$NAME" ]] && [[ "$KIND" == "ConfigMap" ]]; then
      ...
      WS_HOST_SUFFIX_EXPR="s/\"applicationHostSuffix\": \".$CURRENT_WS_HOST_NAME\"/\"applicationHostSuffix\": \".$NEW_WS_HOST_NAME\"/"
      sed -i "$WS_HOST_SUFFIX_EXPR" /tmp/"$NAME"overrides.yaml

      CURRENT_WS_SUFFIX_REGEX=$DEV_BRANCH.$STAGING_HOST_NAME
      # In this, we only do a find replace on a given line if we find applicationHostSuffixRegex on the line
      sed -i -e "/applicationHostSuffixRegex/s/$CURRENT_WS_SUFFIX_REGEX/$DEV_BRANCH\\\\\\\\.staging\\\\\\\\.bhojpur\\\\\\\\.net/g" /tmp/"$NAME"overrides.yaml

      yq m -x -i k8s.yaml -d "$i" /tmp/"$NAME"overrides.yaml
   fi

   # update application-templates configmap to set affinity for application, ghosts, image builders, etc.
   # if this is not done, and they start on a node other than application, they won't be able to talk to registry-facade or ws-daemon
   if [[ "application-templates" == "$NAME" ]] && [[ "$KIND" == "ConfigMap" ]]; then
      WORK="overrides for $NAME $KIND"
      echo "$WORK"
      touch /tmp/"$NAME"overrides.yaml
   ...
   # This changes or removes resources from the configmap we use to uninstall Bhojpur.NET Platform
   ...
   if [[ "bhojpur-app" == "$NAME" ]] && [[ "$KIND" == "ConfigMap" ]]; then
      WORK="overrides for $NAME $KIND"
      echo "$WORK"
      # Get a copy of the config we're working with
      ...
      while [ "$ci" -le "$CONFIG_MATCHES" ]; do
         ...
         #    echo "Avoiding writing the $CONFIG_NAME $CONFIG_KIND to the bhojpur-app ConfigMap"
         #    ci=$((ci + 1))
         #    continue
         # fi

         yq r /tmp/"$NAME"-"$KIND"-original.yaml -d "$ci" > /tmp/bhojpur-app_config_"$ci"
         if [ "$ci" -gt 0 ]; then
         ...
         if [[ "messagebus" == "$CONFIG_NAME" ]] && [[ "$CONFIG_KIND" == "PodDisruptionBudget" ]]; then
            echo "Update new $CONFIG_NAME $CONFIG_KIND in the bhojpur-app ConfigMap"
            yq w -i /tmp/bhojpur-app_config_"$ci" apiVersion $PDB_POLICY_VERSION
         fi

         cat /tmp/bhojpur-app_config_"$ci" >> /tmp/"$NAME"-"$KIND"-overrides.yaml
         ci=$((ci + 1))
         new_ci=$((new_ci + 1))
      done
