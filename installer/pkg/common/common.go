...
import (
	...
	wsk8s "github.com/bhojpur/platform/common-go/kubernetes"
	config "github.com/bhojpur/platform/installer/pkg/config/v1"
...
func DefaultLabels(component string) map[string]string {
	return map[string]string{
		"app":                        AppName,
		"component":                  component,
		wsk8s.BhojpurNodeServiceLabel: component,
	}
}
...
func DefaultEnv(cfg *config.Config) []corev1.EnvVar {
	...
	return []corev1.EnvVar{
		{Name: "BHOJPUR_DOMAIN", Value: cfg.Domain},
		{Name: "BHOJPUR_INSTALLATION_LONGNAME", Value: cfg.Domain},  // todo: figure out these values
		{Name: "BHOJPUR_INSTALLATION_SHORTNAME", Value: cfg.Domain}, // todo: figure out these values
		{Name: "BHOJPUR_REGION", Value: cfg.Metadata.Region},
...
func TracingEnv(cfg *config.Config) (res []corev1.EnvVar) {
	...
	if ep := cfg.Observability.Tracing.Endpoint; ep != nil {
		...
		// TODO(: think about proper error handling here.
...
// If config registry is set to the Bhojpur registry, the third-party registry is returned. If
...
func ThirdPartyContainerRepo(configRegistry string, thirdPartyRegistry string) string {
	...
	if configRegistry == BhojpurContainerRegistry {
