...
var (
	bhojpurBuiltinUserID = "00000000-0000-0000-0000-000000000000"
)
...
func TestUploadUrl(t *testing.T) {
	tests := []struct {
		...
		{
			Name:         "simple name",
			InputOwnerID: bhojpurBuiltinUserID,
			InputName:    "test-blob",
		},
		{
			Name:         "new user",
...
func TestUploadDownloadBlob(t *testing.T) {
	f := features.New("UploadDownloadBlob").
		...
		Assess("it should upload and download blob", func(_ context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			resp, err := bs.UploadUrl(ctx, &content_service_api.UploadUrlRequest{OwnerId: bhojpurBuiltinUserID, Name: "test-blob"})
			if err != nil {
				t.Fatal(err)
			}
			url := resp.Url
			...
			resp2, err := bs.DownloadUrl(ctx, &content_service_api.DownloadUrlRequest{OwnerId: bhojpurBuiltinUserID, Name: "test-blob"})
			if err != nil {
				t.Fatal(err)
			}
			url = resp2.Url
