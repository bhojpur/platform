...
import (
	...
	// Bhojpur.NET Platform uses MySQL, so it makes sense to make this DB driver available
	// by default.
	_ "github.com/go-sql-driver/mysql"

	bhojpur "github.com/bhojpur/platform/bhojpur-protocol"
	wsmanapi "github.com/bhojpur/platform/bp-manager/api"
	"github.com/bhojpur/platform/common-go/log"
...
func NewComponentAPI(ctx context.Context, namespace string, client klient.Client) *ComponentAPI {
	return &ComponentAPI{
		...
		serverStatus: &serverStatus{
			Client: make(map[string]*bhojpur.APIoverJSONRPC),
...
type serverStatus struct {
	...
	Client map[string]*bhojpur.APIoverJSONRPC
}
...
type ComponentAPI struct {
	...
	wsmanStatus struct {
		Port   int
		Client wsmanapi.ApplicationManagerClient
	}
...
// Supervisor provides a gRPC connection to a Bhojpur.NET Platform application's supervisor
func (c *ComponentAPI) Supervisor(instanceID string) (grpc.ClientConnInterface, error) {
	pod, _, err := selectPod(ComponentApplication, selectPodOptions{
...
type bhojpurServerOpts struct {
	User string
}

// BhojpurServerOpt specificies Bhojpur.NET Platform server access
type BhojpurServerOpt func(*bhojpurServerOpts) error

// WithBhojpurUser specifies the user as which we want to access the API.
func WithBhojpurUser(name string) BhojpurServerOpt {
	return func(o *bhojpurServerOpts) error {
		o.User = name
		return nil
	}
}

// BhojpurServer provides access to the Bhojpur.NET Platform server API
func (c *ComponentAPI) BhojpurServer(opts ...BhojpurServerOpt) (bhojpur.APIInterface, error) {
	var options bhojpurServerOpts
	for _, o := range opts {
		err := o(&options)
		if err != nil {
			return nil, xerrors.Errorf("cannot access Bhojpur.NET Platform server API: %q", err)
		}
	}
	...
	var res bhojpur.APIInterface
	err := func() error {
		tkn := c.serverStatus.Token[options.User]
		if tkn == "" {
			var err error
			tkn, err = c.createBhojpurToken(options.User)
			if err != nil {
				return err
			}
		...
		cl, err := bhojpur.ConnectToServer(endpoint.String(), bhojpur.ConnectToServerOpts{
			Token: tkn,
			Log:   log.Log,
		})
		if err != nil {
...
func (c *ComponentAPI) BhojpurSessionCookie(userId string, secretKey string) (*http.Cookie, error) {
	var res *http.Cookie
	err := func() error {
		config, err := GetServerConfig(c.namespace, c.client)
		if err != nil {
...
func (c *ComponentAPI) GetUserId(user string) (userId string, err error) {
	...
	var row *sql.Row
	if user == "" {
		row = db.QueryRow(`SELECT id FROM d_b_user WHERE NOT id = "` + bhojpurBuiltinUserID + `" AND blocked = FALSE AND markedDeleted = FALSE`)
	} else {
...
func (c *ComponentAPI) createBhojpurToken(user string) (tkn string, err error) {
	...
	// see https://github.com/bhojpur/platform/blob/master/components/bhojpur-protocol/src/protocol.ts#L274
	...
	_, err = db.Exec("INSERT INTO d_b_bhojpur_token (tokenHash, name, type, userId, scopes, created) VALUES (?, ?, ?, ?, ?, ?)",
		hashVal,
	...
	c.appendCloser(func() error {
		_, err := db.Exec("DELETE FROM d_b_bhojpur_token WHERE tokenHash = ?", hashVal)
		return err
	})
...
func (c *ComponentAPI) CreateBhojpurOneTimeSecret(value string) (id string, err error) {
...
// ApplicationManager provides access to ws-manager
func (c *ComponentAPI) ApplicationManager() (wsmanapi.ApplicationManagerClient, error) {
	...
	if c.wsmanStatus.Port == 0 {
		...
		pod, _, err := selectPod(ComponentApplicationManager, selectPodOptions{}, c.namespace, c.client)
		if err != nil {
			return nil, err
		}
	...
	c.wsmanStatus.Client = wsmanapi.NewApplicationManagerClient(conn)
	return c.wsmanStatus.Client, nil
}
...
// DB provides access to the Bhojpur.NET Platform database.
...
func (c *ComponentAPI) DB(options ...DBOpt) (*sql.DB, error) {
	...
	db, err := sql.Open("mysql", fmt.Sprintf("bhojpur:%s@tcp(%s:%d)/%s", config.Password, config.Host, config.Port, opts.Database))
	if err != nil {
		return nil, err
	}
...
type ContentService interface {
	...
	csapi.ApplicationServiceClient
}
...
func (c *ComponentAPI) ContentService() (ContentService, error) {
	...
	c.contentServiceStatus.ContentService = cs{
		...
		ApplicationServiceClient: csapi.NewApplicationServiceClient(conn),
	}
