...
func Setup(ctx context.Context) (string, string, env.Environment) {
	var (
		username         string
		waitBhojpurReady time.Duration
	...
	flagset.DurationVar(&waitBhojpurReady, "wait-bhojpur-timeout", 5*time.Minute, `wait time for Bhojpur.NET Platform components before starting integration test`)
	...
	testenv.Setup(
		waitOnBhojpurRunning(e.Namespace(), waitBhojpurReady),
	)

	return username, e.Namespace(), testenv
}

func waitOnBhojpurRunning(namespace string, waitTimeout time.Duration) env.Func {
	klog.V(2).Info("Checking status of Bhojpur.NET Platform components...")
	...
	return func(ctx context.Context, cfg *envconf.Config) (context.Context, error) {
		...
		err := wait.PollImmediate(5*time.Second, waitTimeout, func() (bool, error) {
			for _, component := range components {
				...
				if err != nil {
					klog.Errorf("unexpected error searching Bhojpur.NET Platform components: %v", err)
					return false, nil
				}
			...
			klog.V(2).Info("All the Bhojpur.NET Platform components are running...")
			return true, nil
		})
		if err != nil {
			return ctx, nil
