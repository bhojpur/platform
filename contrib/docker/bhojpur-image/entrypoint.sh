...
BASEDOMAIN=${BASEDOMAIN:-}                          # Used as Bhojpur.NET Platform domain, `bhojpur.` prefix will be added.
DOMAIN=${DOMAIN:-}                                  # Used as Bhojpur.NET Platform domain as is.
REMOVE_NETWORKPOLICIES=${REMOVE_NETWORKPOLICIES:-}  # Remove Bhojpur.NET Platform network policies when set to 'true'.
...
[ -d "/var/bhojpur/docker-registry" ] && chown 1000 /var/bhojpur/docker-registry
[ -d "/var/bhojpur/minio" ] && chown 1000 /var/bhojpur/minio
...
# If we run bhojpur-k3s in a docker-compose setting, we probably want to access
# other containers (e.g. the a local registry or GitLab instance) from within
# the k3s cluster (e.g. from the applications). For this, we need access to
...
# prepare Bhojpur.NET Platform helm installer
BHOJPUR_HELM_INSTALLER_FILE=/var/lib/rancher/k3s/server/manifests/bhojpur-helm-installer.yaml
...
sed 's/^/    /' /values.yaml >> "$BHOJPUR_HELM_INSTALLER_FILE"

sed -i "s/\$DOMAIN/$DOMAIN/g" "$BHOJPUR_HELM_INSTALLER_FILE"
sed -i "s/\$BASEDOMAIN/$BASEDOMAIN/g" "$BHOJPUR_HELM_INSTALLER_FILE"
sed -i "s+\$HELMIMAGE+$HELMIMAGE+g" "$BHOJPUR_HELM_INSTALLER_FILE"
...
# bhojpur-helm-installer.yaml needs access to Kubernetes by the public host IP.
...
installation_completed_hook() {
    # shellcheck disable=SC2143
    while [ -z "$(kubectl get pods | grep bhojpur-helm-installer | grep Completed)" ]; do sleep 10; done

    echo "Removing installer manifest ..."
    rm -f "$1"
}
installation_completed_hook "$BHOJPUR_HELM_INSTALLER_FILE" &


if [ "$REMOVE_NETWORKPOLICIES" = "true" ]; then
    # Remove network policy, temporary fix
    rm /chart/templates/*networkpolicy*.yaml
fi


# start k3s
/bin/k3s server --disable traefik --node-label "bhojpur.net/main-node=true" "$@"
